# 오버라이딩 : 부모 클래스를 상속받아 동일한 메소드에 대해 재정의해 사용하는 것을 오버라이딩이라고 한다.
# 오버로딩 : 파이썬은 오버로딩이 없음 (데이터형이 없어 매개변수를 아무거나 넣어도 동작하므로 오버로딩과 유사한 효과가 있음, 디폴트 매개변수 문법이 있음
...
class TV(ProtoTV): # ProtoTV 로 부터 상속을 받음
    def set_channel(self, cnl): # ProtoTV로 부터 상속을 받음
        if 0 < cnl < 2000:
            self.channel = cnl
            print(f"채널을 {cnl}로 변경 하였습니다.")
        else:
